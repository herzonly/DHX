...
function initializeHelper(bot) {
    ...
    try {
      ...
      if (typeof textOrOptions === 'object' && !Buffer.isBuffer(textOrOptions)) {
        ...
        if (msg.video) {
          ...
          const { text: finalCaption, parseMode: finalParseMode } = safeParseMode(caption, parseMode);
        ...
        if (msg.text) {
          const text = msg.text;
          const { text: finalText, parseMode: finalParseMode } = safeParseMode(text, parseMode);
          
          return await this.telegram.sendMessage(chatId, finalText, {
            parse_mode: finalParseMode,
            reply_to_message_id: replyToMessageId || msg.reply_to_message_id,
            ...msg.options
          });
      ...
      return await sendWithFallback(
        (txt, opts) => this.telegram.sendMessage(chatId, txt, opts),
        text,
        { ...options, reply_to_message_id: replyToMessageId, parse_mode: parseMode }
      );
    } catch (error) {
      try {
  ...
  try {
    ...
    const { text: finalText, parseMode: finalParseMode } = safeParseMode(fullText, parse_mode);
...
bot.removeKeyboard = async function(chatId, text = 'Keyboard removed', options = {}) {
  try {
    ...
    const replyToMessageId = reply_to_message_id || reply_to;
    const { text: finalText, parseMode: finalParseMode } = safeParseMode(text, parse_mode);
...
if (!global.waitingForAnswer) {
  ...
  try {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      const parseMode = question.parse_mode || 'Markdown';

      const { text: finalText, parseMode: finalParseMode } = safeParseMode(questionText, parseMode);
      ...
      if (question.response) {
        ...
        if (typeof responseText === 'function') {
          responseText = responseText(userAnswer, responses);
        ...
        const { text: finalResponse, parseMode: responseParseMode } = safeParseMode(responseText, parseMode);
        await bot.telegram.sendMessage(chatId, finalResponse, {
  ...
  bot.sendList = async function(chatId, text, title, rows = [], options = {}) {
    ...
    const { text: finalText, parseMode: finalParseMode } = safeParseMode(fullText, 'Markdown');
  ...
  bot.editMessageText = async function(text, options = {}) {
    try {
      ...
      const { text: finalText, parseMode: finalParseMode } = safeParseMode(text, parseModeFinal);
      ...
      if (inline_message_id) {
        return await this.telegram.editMessageText(
          ...
          inline_message_id,
          finalText,
          { parse_mode: finalParseMode, ...rest }
        );
      }
  ...
  bot.editMessageCaption = async function(caption, options = {}) {
    try {
      const { chat_id, message_id, inline_message_id, parse_mode, mentions, ...rest } = options;
      const parseModeFinal = parse_mode || 'Markdown';
      const { text: finalCaption, parseMode: finalParseMode } = safeParseMode(caption, parseModeFinal);
...
async function smsg(bot, ctx) {
  ...
  m.replyToQuoted = async function(text, options = {}) {
    ...
    if (m.quoted) {
      return await sendWithFallback(
        (txt, opts) => ctx.telegram.sendMessage(m.chat, txt, opts),
        text,
        { reply_to_message_id: m.quoted.id, parse_mode: parseMode, ...options }
      );
    }
    return await sendWithFallback(
      (txt, opts) => ctx.reply(txt, opts),
...
function safeParseMode(text, parseMode = 'Markdown') {
  if (!text || typeof text !== 'string') return { text, parseMode: null };
...
async function sendWithFallback(sendFunc, text, options = {}) {
  const parseMode = options.parse_mode || 'Markdown';
  
  try {
    const { text: processedText, parseMode: finalParseMode } = safeParseMode(text, parseMode);
    return await sendFunc(processedText, { ...options, parse_mode: finalParseMode });
  } catch (error) {
    if (error.description && error.description.includes('parse')) {
      try {
        return await sendFunc(text, { ...options, parse_mode: null });
      } catch (fallbackError) {
        throw fallbackError;
      }
    }
