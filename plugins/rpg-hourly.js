...
let handler = async (m, { conn = bot, isPrems }) => {
    let time = global.db.data.users[m.sender].lastclaim + 3600000
  if (new Date - global.db.data.users[m.sender].lastclaim < 3600000) return m.reply(`Anda sudah mengklaim, klaim harian hari ini\ntunggu selama ${msToTime(time - new Date())} lagi`)
      //  conn.reply(m.chat, `Anda sudah mengklaim dan mendapatkan :`, m)
       ...
       // global.db.data.users[m.sender].potion += 5
        conn.reply(m.chat, `    *[ H O U R L Y ]*
        
  Selamat kamu mendapatkan:\n\n+${isPrems ? prem : free} Exp\n+${isPrems ? moneyprem : moneyfree} Money`, m)
        global.db.data.users[m.sender].lastclaim = new Date * 1
        setTimeout(() => {
					conn.reply(m.chat, `Hourly sudah bisa di dapatkan kembali`, m.id)
					}, timeout)
    } 
handler.help = ['hourly']
handler.tags = ['rpg']
