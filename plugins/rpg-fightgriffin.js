let handler = async (m, { conn, bot, ctx, isGroup, chatId, userId }) => {
  ...
  if (isBotAdmin) {
    participants = Object.keys(global.db.data.users)
      .filter(uid => {
        ...
        return user && user.griffin && user.griffin > 0 && uid !== userId.toString();
      });
  } else {
    const chat = global.db.data.chats[chatId];
    
    if (!chat || !chat.participants || chat.participants.length < 2) {
      return m.reply('❌ Minimal harus ada 2 member aktif dengan Griffin di grup untuk battle!\n\n💡 Tip: Jadikan bot sebagai admin untuk battle dengan semua member grup.');
    }

    participants = chat.participants.filter(uid => 
      uid !== userId.toString() &&
      global.db.data.users[uid] && 
      global.db.data.users[uid].griffin && 
      global.db.data.users[uid].griffin > 0
    );
  }

  if (participants.length < 1) {
    return m.reply('❌ Tidak ada member lain dengan Griffin di grup untuk battle!');
  }

  let lawan = participants[Math.floor(Math.random() * participants.length)];

  if (!global.db.data.users[lawan] || !global.db.data.users[lawan].griffin) {
    return m.reply('❌ Lawan yang terpilih tidak memiliki Griffin!');
  }
  ...
  m.reply(`*Pet Kamu* (🦚griffin ${global.db.data.users[userId].griffin}) ⚔️ menantang 🦚griffinnya *${lawanName}* (🦚griffin ${global.db.data.users[lawan].griffin}) sedang berkelahi.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yang menang 🎮.`);
  ...
  setTimeout(() => {
    ...
    let kesempatan = [];
    for (i = 0; i < global.db.data.users[userId].griffin; i++) kesempatan.push(userId);
    ...
    if (pointPemain > pointLawan) {
      ...
      m.reply(`*${userName}* [${pointPemain * 10}] - [${pointLawan * 10}] *${lawanName}*\n\n*Pet🦚Kamu* (griffin ${global.db.data.users[userId].griffin}) MENANG melawan 🦚griffinnya *${lawanName}* (griffin ${global.db.data.users[lawan].griffin}) karena griffin🦚kamu ${alasanMenang[Acakin(0, alasanMenang.length - 1)]}\n\nHadiah Rp. ${hadiah.toLocaleString()}\n+1 Tiketcoin`);
      ...
      if (denda === 0) {
        m.reply(`*${userName}* [${pointPemain * 10}] - [${pointLawan * 10}] *${lawanName}*\n\n*Pet🦚Kamu* (griffin ${global.db.data.users[userId].griffin}) KALAH melawan 🦚griffinnya *${lawanName}* (griffin ${global.db.data.users[lawan].griffin}) karena pet kamu ${alasanKalah[Acakin(0, alasanKalah.length - 1)]}\n\nUang kamu tidak berkurang karena sudah habis 😅\n+1 Tiketcoin`);
      } else {
