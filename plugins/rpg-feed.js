let handler = async (m, { conn, args, usedPrefix }) => {
    ...
    switch (type) {
        ...
        case 'naga':
            ...
            if (new Date - global.db.data.users[m.sender].nagalastclaim > 600000) {
                if (global.db.data.users[m.sender].makananpet > 0) {
                    ...
                    global.db.data.users[m.sender].nagalastclaim = new Date * 1 // FIXED: was kudalastclaim
                    conn.reply(m.chat, `Berhasil memberi makan pet ${type}`, m)
                    if (naga > 0) {
            ...
            } else m.reply(`Pet kamu sudah kenyang, coba kasih makan *${waktuuu}* lagi`) // FIXED: was waktuk
            break
        ...
        case 'rubah':
            ...
            if (new Date - global.db.data.users[m.sender].rubahlastclaim > 600000) {
                if (global.db.data.users[m.sender].makananpet > 0) {
                    ...
                    conn.reply(m.chat, `Berhasil memberi makan pet ${type}`, m)
                    if (rubah > 0) { // FIXED: was checking naga instead of rubah
            ...
            } else m.reply(`Pet kamu sudah kenyang, coba kasih makan *${waktuu}* lagi`) // FIXED: was waktuk
            break
        ...
        default:
            return conn.reply(m.chat, `${usedPrefix}feed [kucing | phonix | kuda | naga | rubah]\nContoh penggunaan: *${usedPrefix}feed kucing*`, m)
    }
}
